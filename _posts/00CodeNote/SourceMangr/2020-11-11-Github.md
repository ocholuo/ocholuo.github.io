...
![01-2](/assets/img/post/01-2.png)

---

### TAG

![01-8](/assets/img/post/01-8.png)
```bash
$ git init
$ git add myfile.txt
...
usecase: switch folder into a submodule

```bash
# - Step 1 — Install git-filter-repo
# - Step 2 — Extract each service folder with history into a temp clone
# 1. Make a fresh clone of the current vault repo (don't work on the live copy)
git clone /path/to/vault-repo /tmp/extract-mortgage_monitor
cd /tmp/extract-mortgage_monitor
# 2. Filter repo to only this subfolder — rewrites history so the folder root becomes the repo root
git filter-repo --subdirectory-filter wiki/..
# 3. Verify history was preserved
git log --oneline

# - Step 3 — Create repos on GitHub
# - Step 4 — Push extracted history to each repo
# 4. Add account B remote (use your account B SSH key or token)
git remote add origin git@github.com:account-B/mortgage_monitor.git
# 5. Push all history
git push -u origin main


# - Step 5 — Remove the folders from the vault repo  
# Remove each folder from git tracking (keeps files locally until submodule re-adds them)
git rm -r wiki/..
git commit -m "chore: remove service folders ahead of submodule migration"

# - Step 6 — Re-add each as a git submodule
git submodule add git@github.com:account-B/mortgage_monitor.git wiki/...
git commit -m "chore: add service projects as submodules under account-B"
git submodule status

# Should show each submodule pinned at a commit hash
# Clone test to confirm it works end-to-end
git clone --recurse-submodules /path/to/vault-repo /tmp/test-clone
```
