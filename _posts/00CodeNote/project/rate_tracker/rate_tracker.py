...
def fetch_today_yield() -> float | None:
    try:
        r = requests.get(YAHOO_TODAY, timeout=10, headers={"User-Agent": "Mozilla/5.0"})
        r.raise_for_status()
...
def fetch_and_seed_history():
    ...
    try:
        r = requests.get(
            YAHOO_SINCE_2020, timeout=15, headers={"User-Agent": "Mozilla/5.0"}
        )
        r.raise_for_status()
...
def check_refi_targets(est_mortgage: float):
    ...
    for target in newly_hit:
        ...
        msg = (
            f"Est. 30-yr mortgage is now {est_mortgage:.2f}% -- "
...
def _draw_chart(
    dates, yields, mtg, title_str, out_path, show_cards=True, year_only_x=False
):
    ...
    ax.plot(
        dates, yields, color=YIELD_C, linewidth=1.8, label="10-yr Treasury", zorder=3
    )
    ...
    for target in REFI_TARGETS:
        ...
        ax.text(
            dates[0],
            target + 0.04,
            f"Target {target}%" + (" (hit)" if hit else ""),
            color=color,
            fontsize=7.5,
            va="bottom",
        )
    ...
    # Bottom banner
    tip = (
        "Rates are currently trending down — worth monitoring for a refi window."
        if delta <= 0
        else "Rates are rising — keep monitoring for a dip."
    )
    banner_y = 0.018
    fig.add_artist(
        mpatches.FancyBboxPatch(
...
def build_year_chart(history: list[dict], year: int):
    ...
    title = (
        f"10-Yr Treasury & Mortgage Rate — {year} {label}  ·  Last updated {updated}"
    )
    ...
    _draw_chart(
        ...
        title,
        out_path,
        show_cards=(year == datetime.date.today().year),
    )
    return out_path
