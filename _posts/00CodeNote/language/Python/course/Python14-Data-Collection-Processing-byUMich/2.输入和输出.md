...
> **安全警告: pickle 不是安全的数据格式 (CWE-502)。** 反序列化 pickle 数据等同于执行代码, 而不只是解析数据。
> 机制: 类可以定义 `__reduce__`, pickle 在 load 时会遵守它返回的 (callable, args) 并直接调用, 所以构造好的 payload 能运行攻击者指定的任意函数。
> 因此 **绝不要对不可信或未经认证的来源调用 `pickle.load()` / `pickle.loads()`** — 网络输入、用户上传的文件、攻击者可写入的 cache / message queue 都算。

```python
# DEMONSTRATION ONLY - illustrates the hazard
import os, pickle
...
pickle.loads(pickle.dumps(Exploit()))   # runs: echo pwned
```

安全替代: 普通数据结构 (dict / list / str / int) 用 `json`, 它只解析数据, 不执行代码; 确实需要 Python 对象往返时, 为序列化结果附加 HMAC 签名, 并在 load 之前先校验签名。

同类风险也存在于底层使用 pickle 或在加载时执行代码的接口: 不带 `SafeLoader` 的 `yaml.load`、`numpy.load(allow_pickle=True)`、`pandas.read_pickle`、`joblib.load`、`torch.load`、`dill`、`shelve`、`marshal`。

> 详见专门笔记 `_posts/10CyberAttack/0ApplicationServerAttacks/2026-08-11-CyberAttack-Insecure-Deserialization.md`, 其中有不安全反序列化的攻击面、利用链和防御措施。
