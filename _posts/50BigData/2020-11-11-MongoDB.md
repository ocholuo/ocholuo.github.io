...
- [BigData - MongoDB](#bigdata---mongodb)
  ...
  - [MongoDB feature](#mongodb-feature)
    ...
    - [监控](#监控)
    - [用户管理与角色 User Management \& Roles](#用户管理与角色-user-management--roles)
...
启动Mongod服务，进入到MongoDB安装目录下的bin目录， 然后输入mongostat命令，如下所示：

---

### 用户管理与角色 User Management & Roles

MongoDB 使用基于角色的访问控制（Role-Based Access Control, RBAC）来管理用户权限。每个用户都创建在某个具体的数据库下，并被授予一个或多个角色，角色决定了该用户能执行哪些操作、作用于哪个数据库。

**内置角色 Built-in Roles：**

| 角色 Role | 权限说明 Description |
| --- | --- |
| `read` | 只读：可读取指定数据库中的数据，不能写入 |
| `readWrite` | 读写：可读取并修改指定数据库中的数据 |
| `dbAdmin` | 数据库管理：索引、统计信息等管理操作，不含用户管理 |
| `userAdmin` | 用户管理：可在本数据库内创建、修改用户和角色 |
| `dbOwner` | 数据库所有者：`readWrite` + `dbAdmin` + `userAdmin` 的合集 |
| `readAnyDatabase` | 跨库只读：可读取集群内所有数据库（需在 `admin` 库下授予） |
| `readWriteAnyDatabase` | 跨库读写：可读写集群内所有数据库（需在 `admin` 库下授予） |
| `clusterAdmin` | 集群管理：分片、副本集配置等集群级操作 |
| `root` | 超级用户：几乎拥有所有权限 |
...
- `pwd`：以明文传入，MongoDB 会自动加盐哈希后存储，不保留原始密码。
- `roles`：数组，一个用户可同时持有多个角色，各自作用于不同的数据库。

**查看与修改用户 Viewing / Modifying Users：**
...
// 修改用户角色
db.updateUser("readonly_user", {
  roles: [{ role: "readWrite", db: "mydb" }]
})

// 修改密码
db.changeUserPassword("readonly_user", "new_password")
...
当内置角色的权限粒度不够时（例如只允许某用户对特定集合有写权限），可以创建自定义角色：

```js
db.createRole({
  role: "collectionWriter",
  privileges: [
    {
...
db.createUser({
  user: "orders_writer",
  pwd: "changeme",
  roles: [{ role: "collectionWriter", db: "mydb" }]
})
```

**认证方式 Authentication Mechanisms：**

默认使用用户名/密码（SCRAM）认证。MongoDB 也支持基于 X.509 证书的认证方式（不存储密码），详见 [[50BigData/MongoDB-X509-Auth]]。
...
db["scans"].updateMany(
  { repo: "git@github.example.com:security-team/vulnerable-secrets-code-samples.git"},
  { $set: { is_test_scan: true } }
)

db["scans"].updateMany(
  { repo: "git@github.example.com:security-team/VulnerableJavaApp.git"},
  { $set: { is_test_scan: true } }
)
```
...
![Screenshot 2023-07-03 at 16.10.45](/assets/img/post/Screenshot%202023-07-03%20at%2016.10.45.png)

![Screenshot 2023-07-03 at 16.10.50](/assets/img/post/Screenshot%202023-07-03%20at%2016.10.50.png)
...
![Screenshot 2023-07-05 at 08.24.35](/assets/img/post/Screenshot%202023-07-05%20at%2008.24.35.png)
...
![Screenshot 2023-07-05 at 08.22.06](/assets/img/post/Screenshot%202023-07-05%20at%2008.22.06.png)
...
1. 正确方法中断创建索引进程
   ...
   1. 然后，从返回的内容中找到类似于下面的内容，这串数字就是 pid 进程号；
   1. 最后，杀死 pid 进程
...
![Screenshot 2023-07-03 at 19.29.35](/assets/img/post/Screenshot%202023-07-03%20at%2019.29.35.png)
   ...
   2. 注：上述测试数据systime字段的取值为`1600000000000000~1600500000000000`
      ...
      1. 不指定排序
         ...
         - ```sql
          ...
          - ![Screenshot 2023-07-03 at 21.04.04](/assets/img/post/Screenshot%202023-07-03%20at%2021.04.04.png)
      ...
      2. 指定排序
         ...
         - ```sql
          ...
          - ![Screenshot 2023-07-03 at 21.08.08](/assets/img/post/Screenshot%202023-07-03%20at%2021.08.08.png)

      3. 对systime建立一个单字段的索引
         ...
         - ```sql
          ...
          - ![Screenshot 2023-07-03 at 21.09.21](/assets/img/post/Screenshot%202023-07-03%20at%2021.09.21.png)


      4. 用一个能命中 `{kfuin,index_name}` 这个复合索引的查询语句，同时也带上对 systime 的比较操作。看看能不能同时命中复合索引和systime字段的单字段索引。
         ...
         3. 结论：
          ...
          - ![Screenshot 2023-07-03 at 21.16.02](/assets/img/post/Screenshot%202023-07-03%20at%2021.16.02.png)


      5. 验证下`{kfuin:1, index_name:1, systime:1}`三字段索引是不是能进一步提升性能。
         1. 结论：
          ...
          - ![Screenshot 2023-07-03 at 21.22.18](/assets/img/post/Screenshot%202023-07-03%20at%2021.22.18.png)
...
```
  
![Screenshot 2023-07-03 at 23.24.11](/assets/img/post/Screenshot%202023-07-03%20at%2023.24.11.png)

1. 我们给其中的第5. 6条数据设置 session_id 字段使之满足session_id的匹配条件
...
![Screenshot 2023-07-03 at 23.26.09](/assets/img/post/Screenshot%202023-07-03%20at%2023.26.09.png)
...
![Screenshot 2023-07-04 at 00.01.22](/assets/img/post/Screenshot%202023-07-04%20at%2000.01.22.png)
...
![Screenshot 2023-07-04 at 00.02.51](/assets/img/post/Screenshot%202023-07-04%20at%2000.02.51.png)
...
![Screenshot 2023-07-04 at 00.04.01](/assets/img/post/Screenshot%202023-07-04%20at%2000.04.01.png)
...
![Screenshot 2023-07-04 at 00.04.30](/assets/img/post/Screenshot%202023-07-04%20at%2000.04.30.png)
...
![Screenshot 2023-07-03 at 11.02.05](/assets/img/post/Screenshot%202023-07-03%20at%2011.02.05.png)
...
1. 其他的解决方式，就是提前声明为某一类查询指定特定的索引(索引必须存在)，当有这类查询时，自动使用特定索引，或者从指定的几个索引中选择一个，而不是通过hint显式指定，且不影响其他类型的查询，这就用到了IndexFilter。
...
1. 如果某一类型的查询设定了IndexFilter，那么执行时通过hint指定了其他的index，查询优化器将会忽略hint所设置index，仍然使用indexfilter中设定的查询计划，也就是下面这种情况：
...
1. IndexFilter不会影响其他类型的查询，如下面的查询类型，查询优化器还是按照原来的规则选择最优计划：
