...
4 settings under the <kbd>Unusual User Activity/UEBA</kbd> panel on the bottom portion of the page. The last setting on this page is Populate User Attribution In Alerts Notification User attribution tries to identify the user who made the change to a resource that resulted in a policy violation.



At the top of the page is a section labeled Unusual User Activity/UEBA and there are two settings within this section. The first is Training Model Threshold which can be set to Low, Medium, or High and the second is a setting for Alert Disposition.
