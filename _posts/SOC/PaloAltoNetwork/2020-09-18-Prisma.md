...
The Auto enable new default policies of the type setting allows administrators to specify policies to be enabled by default and can be configured by policy severity.


The last setting: <kbd>Populate User Attribution In Alerts Notification<kbd> User attribution tries to identify the user who made the change to a resource that resulted in a policy violation.
