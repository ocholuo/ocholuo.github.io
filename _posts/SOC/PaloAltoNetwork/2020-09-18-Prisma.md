...
The last setting: <kbd>Populate User Attribution In Alerts Notification User attribution<kbd> tries to identify the user who made the change to a resource that resulted in a policy violation.

<kbd>Unusual User Activity/UEBA</kbd>
- two settings within this section. 
- The first is <kbd>Training Model Threshold</kbd> which can be set to Low, Medium, or High
- the second is a setting for <kbd>Alert Disposition</kbd>.
