...
![](../github-geren/ocholuo.github.io/assets/img/post/2026-04-06-19-39-57.png)

生活例子：“您想去中研院图书馆看一本古籍，但您得先在大门口办证”。

- 角色安排:
  - 用户（User）：就是想看书的太奶奶您。
  - 客户端（Client）：这就是帮您查书的那个小电脑（或手机上的查书软件）。
  - 认证服务器（Auth Server）：这是大门口的办证处（它认识您，有您的照片和档案）。
  - 资源服务器（Resource Server）：这就是藏经阁（书库），里面锁着那本宝贝古籍。

- 新流程：
  - 第一步：您跟小电脑说“我要看书”
    - 您坐在图书馆的小电脑前，点了一下“我要看那本《长生不老经》”。小电脑一看，这书太珍贵了，它没权利带您看，于是它 **“唰”地一下，把您的屏幕跳到了大门口办证处** 的网页。

  - 第二步：您去办证处刷脸（Authentication）
    - 办证处（认证服务器）问：“你是谁呀？”您输入了密码或者刷了脸。办证处一看：“哟，是咱百岁太奶奶！档案对上了。”
    - 这就是 **OIDC** 发挥作用的时候：办证处给了小电脑一张名牌（`ID Token`），上面写着“此人确系太奶奶”。

  - 第三步：办证处问您“准许这台电脑帮您翻书吗？”
    - 办证处接着问：“太奶奶，您准许这台小电脑替您去书库把书调出来，并在屏幕上给您看吗？”您点了个“同意”。
    - 这就是 OAuth 2.0 在征得您的授权。

  - 第四步：办证处发“进出门票”（Access Token）
    - 办证处看您同意了，就给了小电脑一张临时门票。这张票上写着：“准许持票电脑进入书库，调阅《长生不老经》，有效期一小时，不准带出馆。”

  - 第五步：小电脑拿着门票去书库（Resource Server）
    - 小电脑揣着这张门票，一溜烟跑到了藏经阁（书库）。守门的机器卫兵查了一下门票上的防伪钢印，确认是办证处发的，就把书库门打开，把书里的内容传给了小电脑。

  - 第六步：您如愿以偿
    - 小电脑回到您面前，屏幕上显出了那本古籍的内容。您舒舒服服地坐在那儿读博，自始至终，书库并没有问您的密码，它只认办证处发的那张门票。

- 对比:
  - 旧方法: 您每想看一本书，都得把大门密码告诉那台小电脑，万一那电脑里有病毒，您的密码不就丢了吗？
  - 新方法: 您的密码：只有您和办证处知道。小电脑：只拿到了临时门票，它偷不走您的身份，也干不了出格的事。
...
- ref:
  ...
  - [https://oauth2-proxy.github.io/oauth2-proxy/docs/](https://oauth2-proxy.github.io/oauth2-proxy/docs/)
...
- 玩游戏的时候弹出QQ登录，微信登录。
  ...
  - `有资质的游戏运营商`还可以通过玩家的**openid**来获取用户的信息， 例如用户的手机号，网名，年龄等信息。
...
- OIDC
  - <font color=OrangeRed> OIDC 对 OAuth2.0 进行协议进行了扩展 </font>
    - OIDC 遵循 oAuth2.0 协议流程，并在这个基础上提供了`id token`来解决三方应用的用户**身份认证**问题。
...
- 兼容OAuth2，可以使用Access Token控制受保护的API资源

- 可以兼容众多的IDP（身份提供商）作为OIDC的OP来使用
...
4. 同样，App 2 使用 code 换 access_token
