...
- Programs manipulate those `objects`: 
  - performing computation with them, asking them to perform methods.
  - an object has a `state` and a collection of `methods` that can perform.
...
- For example, turtle `objects`
  - each turtle has a `state`: 
    - turtle’s position, color, heading...
  - Each turtle also has the `ability`: 
