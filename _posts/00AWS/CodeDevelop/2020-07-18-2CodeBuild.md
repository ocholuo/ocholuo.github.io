...
2. CodeBuild <font color=red> downloads the source code into the build environment and then uses the build specification (buildspec) </font>
   - A buildspec
   - a collection of build commands and related settings CodeBuild uses to run a build.
   - YAML format
...
## the original docker build process
